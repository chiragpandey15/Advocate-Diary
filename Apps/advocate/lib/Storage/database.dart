...
class DbHelper{
      ...
      for(final i in result)
      {
        ...
        cases.add(
          new Case(
            ...
            i['$messagePermission']==1?true:false,i['$description'],i['$dispose']
          )
        );
      }
    }
      ...
      cases.add(
        new Case(
          ...
          ,i['$opponent'].toString(),i['$opponentAdvocate'].toString(),i['$courtName'].toString(),i['$courtNumber'].toString(),fee,
            i['$messagePermission']==1?true:false,i['$description'].toString(),i['$dispose']
        )
      );
    }
    return cases;
  ...
  Future<List<Case>>getDisposedCase() async{
    await initDatabse();
    final  sql='''SELECT * from $CaseTable WHERE $dispose=1 ORDER BY $caseId DESC''';
    final result=await db.rawQuery(sql);
